...
**Сильно связанный граф** - такой граф, что для любых вершин u и v данного графа существует ориентированный путь из u в v и одновременно ориентированный путь из v в u.

**Компоненты сильной связи** (сильные компоненты орграфа) - максимальные по включению сильно связные подграфы данного графа.
...
6. Одновременно создаётся список смежности транспонированного графа, т.е. вектор векторов *graphT*.
7. Вершины данного графа разбиваются на компоненты сильной связности, или "цвета". Данная информация хранится в векторе векторов *colorGraph*.
...
10. Производится проход по вектору векторов *graph* и сверяется цвет вершин, между которыми есть рёбра.
	- Если ребром связаны вершины разных "цветов", это считается ребром графа конденсации. В таком случае пара "*color\[i\]* *color\[graph\[i\]\[j\]\]*" (где *color\[i\]* - "цвет" *i*-той вершины, а *color\[i\]\[j\]* - цвет *j*-той вершины, связанной с *i*-той вершиной) в виде строки записывается во множество *gk*.
11. Производится вывод списка вершин получившегося графа конденсации, хранящегося в множестве *gk*, в файл (*outputX.txt*).
...
![Работа программы до ввода](rr_img/pratsa2.png)
