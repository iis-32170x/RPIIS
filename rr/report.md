...
1. Подключаются необходимые для работы программы библиотеки C++:
	...
	- *vector* - вектора (динамические массивы)
	- *string* - строки
	- *stack* - стеки
...
10. Производится проход по вектору векторов *graph* и сверяется цвет вершин, между которыми есть рёбра.
	- Если ребром связаны вершины разных "цветов", это считается ребром графа конденсации. В таком случае пара "*color\[i\]* *color\[graph\[i\]\[j\]\]*" в виде строки записывается во множество *gk*.
