...
### Алгоритм программы
 ...
 1. Инициализируем возможность использования библиотеки команд `bash`
    ...
    #!/bin/bash
    ```
 3. Присваиваем переменным `X` `N` `directory` значения, вводимые с клавиатуры, и переменной `q` значение 0
   ```
   directory="$1"
   N="$2"
   X="$3"
...
3. Дикларируем функцию вывода (функция `end()`)
   ...
   ```
   3.1 Выводим (функция `echo`) `"end"`
   ```
   echo "end"
   ```
   3.2 Выход из программы (команда `exit`)
   ```
   exit 0
   ```
5. Дикларируем функцию удаления файлов (функция `out()`)
   ```
   out() {
      ...
   }
   ```
   5.1 Передаем все файлы (команда `ls -l`) в папке `directory` команде `for `
   ```
   ls -l "$directory"
   ```
   5.2 Находим файлы (команда `for /R`) в папке `directory`с любым названием и любым расширением `(/*)`
   ```
   for file in "%directory"/*; do (
   ...
   )
   ```
   5.3 Если счетчик `q` меньше, чем `N`, то переходим к пункту 5.4, если нет, то переходим к пункту 6
6. Дикларирем функцию проверки выполнения функции `out()` и режима сна (функция `w()`)
   ```
   w() {
      q=0
      number=0
   ...
   ```
7. Присваиваем переменной `number` значение 0
   ```
   number=0
   ```
8. Считаем количество файлов (переменная `number`) в папке `directory`
   ```
   for file in "$directory"/*; do
        if [ -f "$file" ]; do
        ((numder++))
   ...
   ```
9. Если количество файлов `number` больше, чем `N` переходм в пункт 4, если нет - переходим в пункт 3
   ```
   if [ "$number" -ge "$N" ]; then
      sleep "X"
      out
