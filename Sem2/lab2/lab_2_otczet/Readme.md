...
```
Потом ко мн-ву применяется функция `add_comas` она нужна для раставления запятых в одиночных кортежах, так как синтаксис питона тип данных кортежей не воспринимает.Пример <1> данный кортеж в питоне будет выглядеть как пустой, а такой <1,> данный кортеж в питоне будет выглядеть
```python
