...
Введите значение элемента
4
Какую операции вы хотите сделать?
...
```
> В функцию приходят указатель на первый узел из списка А (node* a) и указатель на первый узел из списка В (node* b). Далее создаем новый узлел с, который является указателем на первый узел нового списка С, и создаем указатели ta и tb, которые равны указателям a и b соответственно.
```
node* c = new node;
node* ta = a;
node* tb = b;
...
```
	...
	tc->info = ta->info;
```
Далее проходимся по каждому узлу из списка В и "ищем" их при помощи *функции поиска* в списке А и, если этих узлов не находится в списке А, то "вставляем" их при помощи *функции вставки* в список С
```
if (SearchNodeInfo(c, tb->info) != nullptr)
		{
			tb = tb->next;
...
```
> В функцию приходят указатель на первый узел из списка А (node* a) и указатель на первый узел из списка В (node* b). Далее создаем указатели ta и tb, которые равны указателям a и b соответственно и указатели HeadC и TailC, которые являются указателями на начало и конец нового списка С соответственно.
```
node* ta = a;
node* tb = b;
node* HeadC = nullptr;
node* TailC = nullptr;
```
Далее функция проходит по каждому узлу списка А и внутри этого цикла функция проходит по каждому узлу из списка В. 
```
while (ta != nullptr) {
	while (tb != nullptr) {...}
	...
...
```
Внутри второго цикла проверяется, совпадают ли значения info текущих узлов ta и tb. Если значения info совпадают, значит найдено пересечение. 
```
if (ta->info == tb->info) {...}
```
1.В случае обнаружения пересечения создается новый узел с и ему присваивается значение info из ta. 
```
node* c = new node;
c->info = ta->info;
c->next = nullptr;
```
Затем с добавляется в конец списка С (проверяется, является ли с первым узлом пересечения):
1.1. Если HeadC равен nullptr, значит это первый узел, поэтому HeadC и TailC устанавливаются равными c.
```
if (HeadC == nullptr) {
	HeadC = c;
	TailC = c;
...
```
1.2. Если HeadC уже указывает на голову пересечения, то c добавляется в конец третьего списка, устанавливая TailC->next равным c, а затем TailC обновляется, чтобы указывать на c.
```
else {
	TailC->next = c;
	TailC = c;
...
```
2.В случае не обнаружения пересечения берется следующий узел из списка В
```
tb = tb->next;
```
После завершения второго цикла узлу tb присваивается значение b и узлу ta присваивается значение ta->next. 
```
ta = ta->next;
tb = b;
```
По завершении цикла возвращается указатель HeadC, который указывает на начало третьего списка (пересечения). Если пересечение не найдено, HeadС будет равно nullptr.
```
return HeadC;
```
#### Работа функции
...
Пересечение 1 и 2 списка:
2
~~~
## Вывод
В данной работе была разработана библеотека для структуры данных типа "односвязный список", которая включала в себя функции работы с данной структурой. 
