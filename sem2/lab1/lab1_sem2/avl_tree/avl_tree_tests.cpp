...
void testPrintTree() {
    ...
    std::cout << "Тестирование вывода структуры дерева...\n";
    ...
    std::cout << "Проверим выше структуру AVL-дерева на корректность визуально.\n";
}

void testInsertWithPrint() {
    AVLTree tree;

    std::cout << "Тестирование операции вставки с выводом структуры дерева:\n";

    // Печать пустого дерева
    std::cout << "Изначальное пустое дерево:\n";
    tree.printTree();

    // Массив ключей для вставки
    int keys[] = { 3, 1, 4, 2, 5 };
    for (int key : keys) {
        std::cout << "Вставка элемента с ключом: " << key << "\n";
        tree.insert(key);
        tree.printTree();
        std::cout << "\n";
    }
...
void testRemoveWithPrint() {
    ...
    std::cout << "Тестирование операции удаления с выводом структуры дерева:\n";

    // Массив ключей для вставки
    int keysToInsert[] = { 3, 1, 4, 2, 5 };
    std::cout << "Изначальное наполнение дерева:\n";
...
void testFindMinAndMax() {
    ...
    std::cout << "Тестирование поиска минимального и максимального элементов:\n";
    ...
    std::cout << "Структура дерева после вставки элементов:\n";
    tree.printTree();

    // Находим и выводим минимальный и максимальный элементы
    Node* minNode = tree.findMin();
    Node* maxNode = tree.findMax();
    std::cout << "Минимальный элемент в дереве: " << (minNode ? std::to_string(minNode->key) : "Дерево пусто") << "\n";
    std::cout << "Максимальный элемент в дереве: " << (maxNode ? std::to_string(maxNode->key) : "Дерево пусто") << "\n";
}

void testFindPredecessorAndSuccessor() {
    AVLTree tree;

    std::cout << "Тестирование поиска ближайшего меньшего и ближайшего большего элементов:\n";
    ...
    for (int key : testKeys) {
        ...
        std::cout << "Для ключа " << key << ":\n";
        std::cout << "  Predecessor: " << (predecessor ? std::to_string(predecessor->key) : "нет") << "\n";
        std::cout << "  Successor: " << (successor ? std::to_string(successor->key) : "нет") << "\n";
    }
}
