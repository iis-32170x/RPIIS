...
Создание контейнера для записи посещенных вершин - `td::map<int, bool>& visited`. Отмечаем первую и после рекурсивно вызываем функцию для соседа. Предусмотрено условие для несвязного графа - если есть хоть одна непосещённая вершина то DFS запускается из не снова.
