...
Mnozh SimRaznNM(Mnozh MultMn[]) {        //Симметрическая разность
    Mnozh C = MultMn[0];
...
Mnozh ReadFile(Mnozh MultMn[], string path) {
    ifstream file;
    file.open(path);
    int KolSkob = 0;
    int KolSkobKort = 0;
    ...
    Mnozh C = SimRaznNM(MultMn);
    return C;
}

bool ReadFile1(Mnozh MultMn[], string path) {
    ifstream file;
    file.open(path);
    int KolSkob = 0;
    int KolSkobKort = 0;
