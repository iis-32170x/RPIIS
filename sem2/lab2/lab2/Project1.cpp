...
element* set::Insert(element* node,string a) 
{
...
void set::Insert(string a)
{
	root = Insert(root, a);
}
void set::input() 
{
	string mnvo; int ind1 = 0; int ind2 = 0;
	cin.ignore();
agn:
...
set** Y::set::file(set**& mnvo,int& countr)
{
	...
	ifstream file("sets4.txt");
	if (file.is_open())
	{
		while (getline(file, mnv)) 
		{
			if (!mnv.empty())
			{ 
				mnv = check(mnv);
...
void set::output(set* mnvo)
{
	mnvo->output(mnvo->root);
}
