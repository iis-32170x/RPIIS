...
3. Производится открытие файла *file*, название которого было только что введено.
	- При ошибке открытия файла программа выводит предупреждение и завершает свою работу.
...
5. Обе строки переводятся при помощи функции *strToSet* в множества *set1* и *set2* соответственно.
	- Программа сообщает, если при переводе одной или двух строк произошла ошибка, и программа переходит на шаг 8.
6. Производится вывод двух множеств, переведённых в строку методом *setToStr* класса *Mnozh*.
7. Программа поочерёдно выводит:
	1. Является ли *set1* элементом *set2* (используется метод *isElement* класса *Mnozh*).
	2. Является ли *set2* элементом *set1* (используется метод *isElement* класса *Mnozh*).
	3. Является ли *set1* подмножеством *set2* (используется метод *isSubset* класса *Mnozh*).
	4. Является ли *set2* подмножеством *set1* (используется метод *isSubset* класса *Mnozh*).
8. Закрывается *file*.
