...
- `N`: Содержит ограничение по размеру которое передает пользователь.
- `folder`: Содержит имя папки, в который производятся действия алгаритма.
- `size`: Содержит размер обробатываемого файла.
- `smallest`:Содержит адрес самого маленького файла.
- `largest`:Содержит адрес самого большого файла.
