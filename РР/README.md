...
![matrix](https://github.com/iis-32170x/RPIIS/blob/Свиридо_Е/РР/screens/матрица.png)

Полученный граф:

![matrix](https://github.com/iis-32170x/RPIIS/blob/Свиридо_Е/РР/screens/граф1.png)

Алгоритм обрабатывает матрицу А. Именно в этой матрице будут находиться кратчайшие пути между парами вершин. Матрица А - это матрица расстояния. Её размер nxn (где n - количество вершин графа). В нашем пример у графа 3 вершины, поэтому матрица имеет размеры 3x3. На главной диагонали всегда расположены нули. Исходя из веса и направления дуг получаем матрицу: 

![m1](https://github.com/iis-32170x/RPIIS/blob/Свиридо_Е/РР/screens/m1.png)
...
![m2](https://github.com/iis-32170x/RPIIS/blob/Свиридо_Е/РР/screens/m2.png)
...
![m3](https://github.com/iis-32170x/RPIIS/blob/Свиридо_Е/РР/screens/m3.png)

k=3:

![m4](https://github.com/iis-32170x/RPIIS/blob/Свиридо_Е/РР/screens/m4.png)
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        int maxDistance = 0;
        for (int j = 0; j < n; j++) {
            if (i != j && distance[i][j] > maxDistance) {
...
#include <vector>
```
- Включение необходимых заголовочных файлов для работы с вводом-выводом, векторами.

``` c++
const int INF = 999999;
```
- В этой строке определяется константа INF, которая используется для представления бесконечности в контексте алгоритма Флойда.

``` c++
int main() {
    setlocale(0, "");
```
- main() - это функция, с которой начинается выполнение программы. setlocale(0, "") используется для установки локали, чтобы обеспечить корректное отображение символов Unicode в консоли.
...
int n;
    ...
    cin >> n;
```
- В этой части кода запрашивается у пользователя количество вершин в графе и сохраняется в переменную n.

```c++
vector<vector<int>> distance(n, vector<int>(n));
    cout << "Введите матрицу расстояний:" << endl;
```
- Здесь создается двумерный вектор distance размером n x n, который будет использоваться для хранения матрицы расстояний между вершинами графа. Затем пользователю предлагается ввести матрицу расстояний.
...
```
- В этом участке кода происходит ввод матрицы расстояний от пользователя. Если пользователь вводит -1, это интерпретируется как бесконечное расстояние, и значение INF присваивается соответствующему элементу матрицы.
``` c++
    for (int k = 0; k < n; k++) {
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
...
```
- Здесь реализован алгоритм Флойда для поиска кратчайших путей между всеми парами вершин графа. Алгоритм выполняет повторяющиеся проходы по матрице расстояний и обновляет значения, если найден более короткий путь через промежуточную вершину k.
...
```
- В этой части кода вычисляется радиус графа. Для каждой вершины i находится наибольшее расстояние maxDistance до других вершин. Затем наименьшее из этих расстояний присваивается переменной radius, которая и представляет радиус графа.
...
```
- Здесь выводится значение радиуса графа на консоль. Затем функция main() возвращает 0, что означает успешное завершение программы.
