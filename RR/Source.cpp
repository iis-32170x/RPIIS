...
int edgeconnectivity(short int** matrix, int v, bool* proof1, bool* proof2, int min, int edg, bool** edges, int level,int skp,bool*proof,int lvl) {
    if (edg != min) {
        if (skp==level) {
            ...
            }//удаляем по дному ребру(если это ребро уже не удалено).если граф стал несвязным,приравниваем количество удаленных ребер и минимальное значение реберной связности
        }
        else{
            skp++;
        }
    }
    if (edg != min) {// если кол-во ребер равно минимуму,то на след.ур. можно не переходить,а сразу выводить резуьтат
        edg++;
        again:
        if (level!=edg-1) {
            if (edg < min) {
...
int main() {
    ...
    bool* proof1 = new bool[v];//иасссив посещенных вершин(для обхода в ширину)
    bool* proof2 = new bool[v];//массив посещенных на данном обходе вершин (для поиска в ширину)
